...
export default function OrderConfirmationPage() {
  ...
  useEffect(() => {
    if (!orderId) { toast.error('No order ID found'); router.push('/'); return; }
    fetchOrder();
  }, [orderId]);

  const fetchOrder = async () => {
    try {
      // ✅ Use getById — returns full order with all pricing fields
      const res  = await fetch(`${API}/api/v1/ecom-order/getById/${orderId}`, {
        credentials: 'include',
      });
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center">
          ...
          <button onClick={() => router.push('/')} className="px-6 py-3 bg-blue-600 text-white rounded-xl font-bold">Go Home</button>
        </div>
      </div>
    );
  }
  ...
  const timelineSteps = [
    {
      ...
      done: true, colorClass: 'bg-green-100', iconClass: 'text-green-600',
    },
    {
      ...
      colorClass: 'bg-green-100', iconClass: 'text-green-600',
    },
    {
      ...
      colorClass: 'bg-blue-100', iconClass: 'text-blue-600',
    },
    {
      icon: Truck, label: 'Out for Delivery', sub: 'Pending', desc: '',
      done: ['OUT_FOR_DELIVERY', 'DELIVERED'].includes(order.orderStatus),
      colorClass: 'bg-orange-100', iconClass: 'text-orange-500',
    },
    {
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-green-50">
      ...
      {confettiActive && (
        <div className="fixed inset-0 pointer-events-none z-50 overflow-hidden">
          {[...Array(50)].map((_, i) => (
            <div key={i} className="absolute w-2 h-2 bg-gradient-to-r from-blue-500 to-green-500 rounded-full animate-confetti"
              style={{ left: `${Math.random() * 100}%`, top: '-10px', animationDelay: `${Math.random() * 2}s`, animationDuration: `${2 + Math.random() * 2}s` }} />
          ))}
        </div>
      )}
        ...
        <div className="bg-white rounded-2xl shadow-xl p-6 mb-6 border-2 border-green-200">
          <div className="text-center">
            ...
            <div className="flex items-center justify-center gap-4">
              <button onClick={() => toast.success('Invoice download coming soon!')}
              ...
              <button onClick={() => { navigator.clipboard.writeText(order._id); toast.success('Order ID copied!'); }}
                ...
                <Share2 className="w-5 h-5" /> Copy ID
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-xl p-6 mb-6">
            ...
            {order.items.map((item, i) => (
              <div key={i} className="flex justify-between items-center py-3 border-b border-gray-100">
                <div className="flex items-center gap-3 flex-1">
                  {item.image && (
                    <img src={`${API}${item.image}`} alt={item.productName} className="w-12 h-12 object-cover rounded-lg" />
                  )}
                  <div>
                ...
                <p className="font-bold text-gray-900">₹{item.totalCost?.toLocaleString('en-IN')}</p>
              </div>
            ))}
          </div>
          ...
          <div className="border-t border-gray-200 pt-4 space-y-2">
            ...
            <div className="flex justify-between text-sm">
              ...
              <span className={`font-semibold ${order.shippingCharge === 0 ? 'text-green-600' : ''}`}>
                {order.shippingCharge === 0 ? 'FREE' : `₹${order.shippingCharge?.toLocaleString('en-IN')}`}
              </span>
            </div>
            <div className="flex justify-between text-lg border-t border-gray-200 pt-3 mt-2">
              <span className="font-bold">Total Paid</span>
              <span className="font-bold text-blue-600">₹{order.totalAmount?.toLocaleString('en-IN')}</span>
            </div>
          </div>
          ...
          <div className="mt-4 pt-4 border-t border-gray-200 space-y-1">
            ...
            <div className="flex justify-between text-sm">
              ...
              <span className={`font-bold ${order.paymentDetails?.status === 'PAID' ? 'text-green-600' : 'text-yellow-600'}`}>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <ul className="text-sm text-gray-700 space-y-1">
                ...
                {order.estimatedDelivery && <li>• Expected delivery: {formatDate(order.estimatedDelivery)}</li>}
